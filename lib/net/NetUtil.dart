...
//  }
  ...
  static Future<Map<String, dynamic>> getJson<T>(
      ...
      _httpJson("get", uri, data: paras).then(errorTransform);
  ...
  static Future<Map<String, dynamic>> postJson(
      ...
      _httpJson("post", uri, data: body).then(errorTransform);

  static Future<Map<String, dynamic>> deleteJson<T>(
          String uri, Map<String, dynamic> body) =>
      _httpJson("delete", uri, data: body).then(errorTransform);

  /// requestBody (json格式参数) 方式的 put
  static Future<Map<String, dynamic>> putJson<T>(
          String uri, Map<String, dynamic> body) =>
      _httpJson("put", uri, data: body).then(errorTransform);

  /// 表单方式的 put
  static Future<Map<String, dynamic>> putForm<T>(
          String uri, Map<String, dynamic> body) =>
      _httpJson("put", uri, data: body, dataIsJson: false).then(errorTransform);
  ...
  static Future<T> errorTransform<T>(Response<Map<String, dynamic>> resp) {
    if (resp.data != null) {
      if (resp.data["code"] == 0) {
    ...
    if (resp.data != null && resp.data["code"] != 0) {
      if (resp.data['data'] != null) {
        ...
        error = HttpError(resp.data["code"], realData['codeMessage']);
      } else {
        /// 失败时  错误提示在 message中时
        error = HttpError(resp.data["code"], resp.data["message"]);
      }
